...
# -*- coding: utf-8 -*-
"""
Quick analysis of student predictions
"""
import matplotlib
...
sns.boxplot(data=df, x='Tier', y='Probability', ax=ax3)
...
sns.histplot(data=df, x='Aptitude', hue='Tier', ax=ax5)
...
sns.violinplot(data=df, x='Tier', y='Communication', ax=ax6)
...
sns.violinplot(data=df, x='Tier', y='Technical', ax=ax7)
...
sns.barplot(data=df, x='Tier', y='Internship', ax=ax8)
...
sns.barplot(data=df, x='Tier', y='Projects', ax=ax9)
...
for idx, row in tier1.iterrows():
    print(f"  - {row['Name']}: {row['Probability']:.1%} probability, CGPA: {row['CGPA']:.2f}")

print("\n[TIER-2 STUDENTS] (3 students)")
...
print("\n[BELOW TIER-3 STUDENTS] (1 student)")
